...
@ComponentScan(basePackages = {"com.feldsher"})
public class ConsoleChatApplication {
    ...
    public static void main(String[] args) {
        ...
        ConfigurableApplicationContext context = SpringApplication.run(ConsoleChatApplication.class, args);
            ...
            try {
                ...
                if ("/status".equalsIgnoreCase(input)) {
                    if (currentConversationId == null) {
                        ...
                        ConversationContext ctx = chatService.getContext(currentConversationId);
                        if (ctx != null) {
                            System.out.println("\n【系统】对话状态：");
                            System.out.println("  - 对话ID: " + ctx.getConversationId());
                            System.out.println("  - 当前阶段: " + (ctx.getCurrentPhase() != null ? ctx.getCurrentPhase().getDescription() : "未知"));
                            System.out.println("  - 已问问题数: " + ctx.getQuestionCount());
                            System.out.println("  - 当前意图: " + (ctx.getCurrentIntent() != null ? ctx.getCurrentIntent().getDescription() : "未识别"));
                            System.out.println("  - 当前Agent: " + (ctx.getCurrentAgent() != null ? ctx.getCurrentAgent().getName() : "无"));
                            System.out.println("  - 当前Skill: " + (ctx.getCurrentSkill() != null ? ctx.getCurrentSkill().getName() : "无"));
                            System.out.println("  - 消息总数: " + (ctx.getMessages() != null ? ctx.getMessages().size() : 0));
                            System.out.println("  - 活动状态: " + (ctx.isActive() ? "活动中" : "已结束"));
                        } else {
                            System.out.println("\n【系统】对话不存在或已被清除。");
                        }
                    }
                ...
                if (currentConversationId == null) {
                    ConversationContext ctx = chatService.createConversation();
                    currentConversationId = ctx.getConversationId();
